...
function renderOrdersTab() {
  ...
  return `
  <table>
    <thead><tr><th>Order</th><th>Source</th><th>Customer</th><th>Items</th><th>Total</th><th>Status</th><th></th></tr></thead>
    <tbody>
      ${state.orders.map(o => `
      <tr>
        <td>${escapeHtml(o.externalId || o.id)}<div class="muted">${new Date(o.createdAt).toLocaleString()}</div></td>
        ...
        <td><select onchange="setOrderStatus('${o.id}', this.value)">${['new','contacted','won','lost'].map(s=>`<option value="${s}" ${o.status===s?'selected':''}>${s}</option>`).join('')}</select></td>
      </tr>`).join('')}
    </tbody>
  </table>`;
}
  ...
  return sorted.map(c => `
    <div class="panel">
      <div class="section-head">
        <div><h3 style="margin:0;">${escapeHtml(c.name)}</h3><span class="muted">${escapeHtml(c.phone)}${c.address ? ' · ' + escapeHtml(c.address) : ''} · via ${escapeHtml(c.source||'—')}</span></div>
