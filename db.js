...
const REDIS_KEY = process.env.UPSTASH_DATA_KEY || 'quivercrm:data';
...
const DEFAULT_DATA = {
  ...
  orders: [],
  tasks: [],
  activities: [], // { id, actorId, actorName, action, detail, createdAt }
};
...
async function loadFromUpstash() {
  ...
  try {
    ...
    data = { ...JSON.parse(JSON.stringify(DEFAULT_DATA)), ...parsed };
  } catch (e) {
