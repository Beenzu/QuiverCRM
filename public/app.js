...
const state = {
  ...
  creatingUser: false,
  editingTaskId: null,
  pipelineStage: null, // null = pipeline summary view; 'new'|'contacted'|'won'|'lost' = drilled into that stage
  activities: [],
  report: null, // computed on demand by the one "Generate Report" button
};
...
async function loadAppData() {
  ...
  if (state.me.role === 'admin') { calls.push(api('/api/admin/users')); calls.push(api('/api/admin/activities')); }
  const results = await Promise.all(calls);
  state.orders = results[0];
  state.customers = results[1];
  state.tasks = results[2];
  if (state.me.role === 'admin') { state.users = results[3]; state.activities = results[4]; }
}
...
function renderApp(app) {
  ...
  <div class="wrap">
    ...
    <div class="tabbar">
      <button class="${state.tab==='orders'?'active':''}" onclick="state.tab='orders'; render();">Pipeline</button>
      ...
      ${isAdmin ? `<button class="${state.tab==='activity'?'active':''}" onclick="state.tab='activity'; render();">Activity</button>` : ''}
      ${isAdmin ? `<button class="${state.tab==='reports'?'active':''}" onclick="state.tab='reports'; render();">Reports</button>` : ''}
    </div>
    ...
    ${state.tab === 'activity' && isAdmin ? renderActivityTab() : ''}
    ${state.tab === 'reports' && isAdmin ? renderReportsTab() : ''}
  </div>
  ${toastHtml()}`;
}
...
function openPipelineStage(stage) { state.pipelineStage = stage; render(); }
function closePipelineStage() { state.pipelineStage = null; render(); }

function renderOrdersTab() {
  return state.pipelineStage ? renderPipelineStageDetail() : renderPipelineSummary();
}
...
function renderPipelineSummary() {
  ...
  <div class="pipeline-grid">
    ${PIPELINE_STAGES.map(s => {
      const count = state.orders.filter(o => o.status === s.key).length;
      return `<button class="pipeline-tile ${s.key}" onclick="openPipelineStage('${s.key}')">
...
function renderPipelineStageDetail() {
  const stage = PIPELINE_STAGES.find(s => s.key === state.pipelineStage);
  const filtered = state.orders.filter(o => o.status === state.pipelineStage);
  const header = `<div class="section-head"><h2>${stage.label} <span class="muted">(${filtered.length})</span></h2><button class="btn small ghost" onclick="closePipelineStage()">← All stages</button></div>`;
  if (filtered.length === 0) return header + `<div class="empty-state">No orders in this stage.</div>`;
  return header + filtered.map(orderCardHtml).join('');
}
 ...
 * Shows what CRM users are doing as they do it — every task and order
 * status change is logged server-side (see logActivity in server.js).
 */
function renderActivityTab() {
  if (state.activities.length === 0) return `<div class="section-head"><h2>Activity</h2></div><div class="empty-state">No activity recorded yet.</div>`;
  return `
    <div class="section-head"><h2>Activity</h2></div>
    <div class="panel">
      ${state.activities.map(a => `
        <div class="review-item">
          <div class="review-head"><strong>${escapeHtml(a.actorName)}</strong><span class="muted">${new Date(a.createdAt).toLocaleString()}</span></div>
          <div class="muted">${escapeHtml(a.action)}${a.detail ? ' — ' + escapeHtml(a.detail) : ''}</div>
        </div>`).join('')}
    </div>`;
}

/* ---------------- Reports (admin-only) ----------------
 * One button, computed from orders already loaded in memory — no separate
 * endpoint needed. "Generate Report" re-pulls the latest orders first, in
 * case other users have changed pipeline stages since this page loaded.
 */
async function generateReport() {
  try {
    state.orders = await api('/api/orders');
  } catch (e) { showToast(e.message); return; }
  const won = state.orders.filter(o => o.status === 'won');
  const lost = state.orders.filter(o => o.status === 'lost');
  const inProgress = state.orders.filter(o => o.status === 'new' || o.status === 'contacted');
  state.report = {
    totalOrders: state.orders.length,
    wonCount: won.length,
    wonValue: won.reduce((s, o) => s + (Number(o.total) || 0), 0),
...
function renderReportsTab() {
  ...
  return `
    ...
    <div class="panel">
      ...
      ${r ? `
      <div class="pipeline-grid" style="margin-top:18px;">
        <div class="pipeline-tile"><span class="pipeline-count">${r.totalOrders}</span><span class="pipeline-label">Total Orders</span></div>
        <div class="pipeline-tile won"><span class="pipeline-count">${r.wonCount}</span><span class="pipeline-label">Won</span></div>
        <div class="pipeline-tile lost"><span class="pipeline-count">${r.lostCount}</span><span class="pipeline-label">Lost</span></div>
        <div class="pipeline-tile"><span class="pipeline-count">${r.inProgressCount}</span><span class="pipeline-label">In Progress</span></div>
      </div>
      <div class="total-row" style="margin-top:16px;"><span>Total sold (won orders)</span><span>${money(r.wonValue, '')}</span></div>
      ` : `<p class="muted" style="margin-top:12px;">Click Generate Report to see what's been sold, won, and lost.</p>`}
    </div>`;
}
