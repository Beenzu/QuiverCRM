...
// Records what a CRM user did, for the admin-only Activity feed. Called
// right before the caller's own `store.save()`, so it persists in the same
// write — no extra save needed. Capped so the log can't grow forever.
function logActivity(data, actor, action, detail) {
  data.activities.unshift({
    id: newId('ACT'), actorId: actor.id, actorName: actor.name, action,
    detail: cleanText(detail, 300), createdAt: new Date().toISOString(),
  });
  if (data.activities.length > 500) data.activities.length = 500;
}
// Constant-time string compare for secrets (setup key, integration key) so
// response timing can't leak how many leading characters an attacker guessed
// correctly. Regular !== / === comparisons short-circuit on the first
// mismatched byte, which is a (minor, but free to close) timing side channel.
function safeEqual(a, b) {
  const bufA = Buffer.from(String(a ?? ''));
  const bufB = Buffer.from(String(b ?? ''));
  if (bufA.length !== bufB.length) {
    crypto.timingSafeEqual(bufA, bufA); // burn comparable time either way
    return false;
  }
...
function issueSession(res, user) {
  const token = jwt.sign({ id: user.id, username: user.username, role: user.role }, JWT_SECRET, { expiresIn: '7d', algorithm: 'HS256' });
...
function requireIntegrationKey(req, res, next) {
  ...
  if (!key || !safeEqual(key, INTEGRATION_KEY)) return res.status(401).json({ error: 'Invalid or missing integration key' });
  next();
}
...
app.get('/api/admin/activities', requireAuth, requireAdmin, (_req, res) => {
  res.json(store.getData().activities);
});
...
app.patch('/api/orders/:id', requireAuth, async (req, res) => {
  ...
  order.status = status;
  logActivity(data, req.user, 'moved order', `${order.externalId || order.id} → ${status}`);
  await store.save();
  res.json(order);
});
...
app.post('/api/tasks', requireAuth, requireAdmin, async (req, res) => {
  ...
  logActivity(data, req.user, 'created task', `"${task.title}"${assignee ? ' → assigned to ' + assignee.name : ''}`);
  await store.save();
  res.json(task);
});
app.patch('/api/tasks/:id', requireAuth, async (req, res) => {
  ...
  if (!isAdmin) {
    ...
    if (status !== undefined) {
      ...
      task.status = status;
      logActivity(data, req.user, 'updated task status', `"${task.title}" → ${status}`);
    }
  } else {
    ...
    if (status !== undefined) { if (!['open', 'in_progress', 'done'].includes(status)) return res.status(400).json({ error: 'Invalid status' }); task.status = status; logActivity(data, req.user, 'updated task status', `"${task.title}" → ${status}`); }
    ...
    if (assignedTo !== undefined) {
      if (!assignedTo) { task.assignedTo = null; task.assignedToName = null; }
        ...
        logActivity(data, req.user, 'reassigned task', `"${task.title}" → ${assignee.name}`);
      }
    }
  }
...
app.delete('/api/tasks/:id', requireAuth, requireAdmin, async (req, res) => {
  ...
  if (task) logActivity(data, req.user, 'deleted task', `"${task.title}"`);
  await store.save();
  res.json({ ok: true });
});
...
app.post('/api/integrations/orders', integrationLimiter, requireIntegrationKey, async (req, res) => {
  ...
  const items = Array.isArray(b.items) ? b.items.slice(0, 50).map(i => ({
    id: cleanText(i?.id, 100), name: cleanText(i?.name, 150), category: cleanText(i?.category, 60),
  ...
  const order = {
    ...
    customerName, phone, altPhone: normalizePhone(b.altPhone),
    town: cleanText(b.town, 100), province: cleanText(b.province, 60),
    address: cleanText(b.address, 500),
    deliveryDate: cleanText(b.deliveryDate, 20), deliveryWindow: cleanText(b.deliveryWindow, 60),
  ...
  if (customer) {
    ...
    if (order.town) customer.town = order.town;
    if (order.province) customer.province = order.province;
...
store.init()
  .then(() => app.listen(PORT, () => console.log(`QuiverCRM running on http://localhost:${PORT}`)))
